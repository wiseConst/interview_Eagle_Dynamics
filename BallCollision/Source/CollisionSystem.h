...
{
    CollisionResult(const sf::Vector2f& normal, const float force, const float distance)
        : m_Normal(normal), m_Force(force), m_Distance(distance)
    {
    }
...
class CollisionSystem final
{
  public:
    ...
    {
        m_CollisionTree = std::make_unique<QuadTree<>>(0, sf::FloatRect{{0, 0}, {screenBounds.x, screenBounds.y}}, nullptr);
    }
    ...
    void SolveStaticCollisions(std::vector<Ball>& balls)
    {
        ...
        for (auto& ball : balls)
        {
            ...
            for (Ball* overlappedBall : overlappedBalls)
            {
                ...
                ball.m_Position -= normal * force;
                ball.UpdateBounds();

                overlappedBall->m_Position += normal * force;
  ...
  private:
    std::unique_ptr<QuadTree<>> m_CollisionTree                 = nullptr;
    ...
    FORCEINLINE std::optional<CollisionResult> AreBallsColliding(const Ball& lhs, const Ball& rhs) const
    {
        ...
        const float force         = 0.5f * (distance - radiusSum);
        return std::make_optional<CollisionResult>(normal, force, distance);
    }
};
